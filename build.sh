#!/bin/bash
# Builds GetDuo.app. Set SIGN_ID to a Developer ID to ship; defaults to ad-hoc for local use.
set -euo pipefail
cd "$(dirname "$0")"
...
xcrun -sdk macosx metal -O -c Sources/GetDuo/Shader.metal -o build/Shader.air
xcrun -sdk macosx metallib build/Shader.air -o build/default.metallib

rm -rf "$APP"
mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources"
cp "$BIN/GetDuo" "$APP/Contents/MacOS/GetDuo"
